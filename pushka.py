...
class Target():
    ...
    def move(self):
        self.x = self.x0 + self.d * math.cos(self.alpha)
        self.y = self.y0 + self.d * math.sin(self.alpha)
        self.alpha += self.w/FPS
...
while not finished:
    ...
    for b in balls:
        b.draw()
    tscore = f.render(f'{target1.point+target2.point}', True, BLACK)
